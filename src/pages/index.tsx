          ...
          <form onSubmit={onSubmit}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 8 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <input name="origin" placeholder="Origine (ex. YUL)"
                       style={{ border: "1px solid #d1d5db", borderRadius: 8, padding: "10px 12px", background: "#fff" }} />
                <input name="dest" placeholder="Destination (ex. CDG)"
                       style={{ border: "1px solid #d1d5db", borderRadius: 8, padding: "10px 12px", background: "#fff" }} />
              </div>
              <button type="submit" disabled={status === "sending"}
                      style={{ background: "#0f172a", color: "white", border: "1px solid #0f172a", borderRadius: 8, padding: "10px 14px", fontWeight: 700, cursor: "pointer", opacity: status === "sending" ? 0.7 : 1 }}>
                {status === "sending" ? "Envoi..." : "S’abonner"}
              </button>
              {status !== "idle" && <small style={{ color: status === "ok" ? "#065f46" : "#7f1d1d" }}>{message}</small>}
            </div>
          </form>
        </div>
      </section>
